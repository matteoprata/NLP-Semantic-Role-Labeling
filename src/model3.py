...
def setup_model3(nn_architecture, embeddings_matrix, data_dictionaries):
    ...
    senses_ids = tf.placeholder(tf.int32, shape=[None, None])           # shape = [BATCH_SIZE, TIME-STEP]
    ...
    pretrained_sense_embs = tf.Variable(embeddings_matrix[1], dtype=tf.float32, trainable=False)  # shape = [VOCAB-SIZE, LEMMA_EMBEDDINGS_SIZE]
    ...
    lookedup_senses = tf.nn.embedding_lookup(pretrained_sense_embs, senses_ids)

    # WORLD REPRESENTATION: concatenation of the lemma embedding, a bit to identify the predicate, the lex and pos encoding, the sense embedding
    # shape = [BATCH_SIZE, TIME-STEP, (LEMMA_EMBEDDINGS_SIZE + 1 + ENC_LEN_LEX + ENC_LEN_POS + SENSE_EMBEDDINGS_SIZE)]
    # word_representation = tf.concat([lookedup_embs, encoding_lex, encoding_pos, lookedup_senses], axis=-1)
    ...
    # shape = [BATCH_SIZE, TIME-STEP, (2*HIDDEN_SIZE) * 2 + 2 + SENSE_EMBEDDING_SIZE]
    to_classify = tf.concat([context_rep, repl_preds_context, lookedup_senses], axis=-1)

    # shape = [N_WORDS, (2*HIDDEN_SIZE) * 2 + 2]
    # to_classify_flat = tf.reshape(to_classify, [-1, (2 * HIDDEN_SIZE) * 2 + 2])

    # shape = [N_WORDS, (2*HIDDEN_SIZE) * 2 + 2 + SENSE_EMBEDDING_SIZE]
    to_classify_flat = tf.reshape(to_classify, [-1, (2 * HIDDEN_SIZE) * 2 + 2 + SENSE_EMBEDDING_SIZE])

    # -------------------------------------------- CLASSIFIER  ------------------------------------------------------

    # W = tf.get_variable("W", shape=[(2 * HIDDEN_SIZE) * 2 + 2, N_ROLES], dtype=tf.float32)
    W = tf.get_variable("W", shape=[(2 * HIDDEN_SIZE) * 2 + 2 + SENSE_EMBEDDING_SIZE, N_ROLES], dtype=tf.float32)
